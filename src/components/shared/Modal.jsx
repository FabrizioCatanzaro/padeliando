export default function Modal({ title, message, children, confirmText = "Confirmar", confirmDanger = false, confirmDisabled = false, hideCancel = false, onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-1000 p-5">
      <div className="bg-surface border border-border-strong rounded-[10px] p-6 max-w-105 w-full">
        ...
        {children
          ? <div className="text-md text-secondary leading-relaxed font-sans">{children}</div>
        ...
        <div className="flex gap-2.5 justify-end mt-5">
          ...
          <button
            ...
            className={`border-0 px-5 py-2.5 font-condensed font-bold text-sm tracking-wide cursor-pointer rounded whitespace-nowrap text-base disabled:opacity-50 disabled:cursor-not-allowed ${confirmDanger ? 'bg-danger' : 'bg-brand'}`}
          >
            {confirmText}
          </button>
        </div>
