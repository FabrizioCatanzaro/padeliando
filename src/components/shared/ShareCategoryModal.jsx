...
/**
 * Comparte una categoría: link por WhatsApp, copiar o menú del sistema.
 * Mismo diseño que ShareModal. `clubName` y `tournamentsCount` son opcionales:
 * una categoría recién creada no tiene ni club ni jornadas que anunciar.
 */
export default function ShareCategoryModal({ categoryName, clubName, tournamentsCount = 0, url, onClose }) {
  ...
  const detail = tournamentsCount > 0
    ? `🎾 ${tournamentsCount} ${tournamentsCount === 1 ? 'torneo' : 'torneos'}`
    : null;
  const lines = [clubName ? `📍 ${clubName}` : null, detail].filter(Boolean);

  // WhatsApp interpreta *texto* como negrita
  const waText    = [`¡Sumate a la categoría *${categoryName}* en Padeleando!`, ...lines, `\n🔗 ${url}`].join('\n');
  const plainText = [`¡Sumate a la categoría ${categoryName} en Padeleando!`, ...lines, `\n🔗 ${url}`].join('\n');
  ...
  const moreOptions = async () => {
    if (canNativeShare) {
      try { await navigator.share({ title: categoryName, text: plainText, url }); } catch { /* usuario canceló */ }
    } else {
      copyLink();
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-center justify-between px-5 pt-4 pb-3 border-b border-border-mid">
          ...
          <span className="font-condensed font-bold text-xl text-white">Compartir categoría</span>
          <button
            onClick={onClose}
        ...
        <div className="px-5 pb-5">
          ...
          <div className="bg-surface-alt border border-border-mid rounded-xl px-4 py-3">
            <p className="text-[13px] text-content leading-relaxed font-sans">
              ¡Sumate a la categoría <strong className="text-white font-semibold">{categoryName}</strong> en Padeleando!
            </p>
            {lines.map((l) => (
